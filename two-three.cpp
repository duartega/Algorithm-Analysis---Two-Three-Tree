...
void TTT::insertHelper(const string &x, int line, node *&t, int &distWord)
{
	...
	else if (x < t->lval && t->left != NULL)
		insertHelper(x, line, t->left, distWord);
	else if (x > t->rval && t->right != NULL)
	...
	else if (t->lval != "" && t->rval != "")
	{
		...
		else
		{
			...
			if (x < t->lval && t->left == NULL)
			{
				if (t->lval < t->parent->lval && t->parent->rval == "")
				{
				...
				else
				{
					...
					node *r = new node(t->lval, "", "", NULL, NULL, NULL, t);
